...
def summary_grades_2021_UK(file_name, academy):
    ...
    try:
        if bool(academy) == True:
            ...
            # reverse index
            merge_DMPF_rev = merge_DMPF[::-1].reset_index(drop = True)
            
            return merge_DMPF_rev
            ...
            # slice the data frame horizontally (extract rows for each grade)
            # This section will filter out each grade - Distinction, Merit, Pass, Fail
            renameData_D = df_stringFileName.query('`Percentage` <= 0.0')
            renameData_M = df_stringFileName.query('`Percentage` <= 0.0')
            renameData_P = df_stringFileName.query('`Percentage` <= 0.0')
            renameData_F = df_stringFileName.query('`Percentage` <= 0.0')
