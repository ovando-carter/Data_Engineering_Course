# slide 98 - Activity 4b
'''
Create a copy of the Activity 4a script created earlier and
rename it to Activity 4b.
Change the protected class constant annual_interest_rate to
be PRIVATE, and set it to 0.001 (0.1%). Ensure it can only
be changed by the method set_annual_interest_rate().

Test the class constant annual_interest_rate to ensure that
it cannot be changed directly from an object, and that can be
changed using the set_annual_interest_rate() method.
'''
class BankAccount():
    # class attributes
    no_accounts = 0
    ...
    def annual_interest_rate(self):
        return self._BankAccount__annual_interest_rate
